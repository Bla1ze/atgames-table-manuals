# AtGames Table Manuals

A complete collection of **user manuals for AtGames pinball tables** — downloadable PDF guides for various tables compatible with AtGames platforms.

## 📂 What’s Included

This repo currently contains a variety of user manuals for many AtGames pinball tables, such as:

- Dr. Seuss Pinball Pack 2
- Zaccaria Primal Carnage (Retro & Deluxe)
- Natural History 2 & 3
- Baby Shark
- Firefighter Wildlands
- Rare Pinball Table
- TMNT Pinball Table
- Zaccaria Blackbelt Deluxe
- Zaccaria Earth Wind & Fire Deluxe
- Zaccaria Future World Deluxe
- Zaccaria Granada Deluxe
- Zaccaria Magic Castle Deluxe
- Zaccaria Mexico Deluxe
- Zaccaria Moon Flight Deluxe
- Zaccaria Pinball Space Shuttle Deluxe  
  *(and many more)*
  
📄 These PDF files are stored directly in the repo for easy access and distribution.

## 📌 How to Use

1. Browse the repository files to locate the manual you need.
2. Click the PDF file name to open it on GitHub.
3. Download the manual using the **Download** button.
4. Use the PDF to reference gameplay instructions for your AtGames pinball table.

## Please note: All these files have been shared by AtGames themselves and are publicly available elsewhere.
